	...
	protected function blockFencedCode( $Line ) {
		if ( preg_match( '/^[' . $Line['text'][0] . ']{3,}[ ]*([^`]+)?[ ]*$/', $Line['text'], $matches ) ) {
			...
			if ( isset( $matches[1] ) ) {
				...
				$class = 'lang-' . $language . ' language-' . $language . ( $language !== 'mermaid' ? ' ' . $language : '' );
	...
	protected function element( array $Element ) {
		...
		if ( isset( $text ) ) {
			...
			if ( isset( $Element['handler'] ) ) {
				$tmp_markup = $this->{$Element['handler']}( $text, $Element['nonNestables'] );
				if ( $Element['name'] !== 'li' || ! preg_match( '#^\s*\[[x\s]{1}\]#', $tmp_markup ) ) {
					$markup .= '>';
					$markup .= $tmp_markup;
				} else {
					$markup    .= ' style="margin-left:-1.25em;list-style-type:none">';
					$tmp_markup = preg_replace( '#^\[\s\]#', '<input type="checkbox" disabled="disabled" />', $tmp_markup );
					$markup    .= preg_replace( '#^\[x\]#', '<input type="checkbox" disabled="disabled" checked="checked" />', $tmp_markup );
				}
