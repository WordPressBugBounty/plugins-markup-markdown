<?php
/**
 * Stable addons toolbar media utilities
 *
 * @category   Addons
...
// Called multiple times, do not force escape this addon.

/**
 * A simple utility to manage the easyMDE toolbar buttons
 */
final class ToolbarEasyMDE {
	...
	protected $prop = array(
		'default_buttons' => array(
			...
			'mmd_wpsimage'        => array( // wps as WordPress image.
	 ...
	 * Start
	 *
	 * @param string $json absolute path to the config file.
	 * @return void
	 */
	...
	public function initialize( $json = '' ) {
		if ( ! markup_markdown()->exists( $json ) ) :
			$toolbar_conf = array( 'mmd_bold', 'mmd_italic', 'mmd_heading', 'mmd_spell_check', 'mmd_pipe', 'mmd_quote', 'mmd_unordered_list', 'mmd_ordered_list', 'mmd_pipe', 'mmd_link', 'mmd_wpsimage', 'mmd_table', 'mmd_pipe', 'mmd_fullscreen', 'mmd_side_by_side', 'mmd_preview', 'mmd_guide' );
			markup_markdown()->put_contents( $json, '{"my_buttons":' . wp_json_encode( $toolbar_conf ) . '}' );
		endif;
		markup_markdown()->clear_cache( $json );
		$toolbar_conf = markup_markdown()->json_decode( $json, false );
		...
		endforeach;
		$ext_buttons = apply_filters( 'markup_markdown_toolbar_buttons', array() );
		...
		foreach ( $toolbar_conf->my_buttons as $button_slug ) :
			if ( ! in_array( $button_slug, $this->prop['active_buttons'], true ) && isset( $this->prop['default_buttons'][ $button_slug ] ) ) :
				$this->prop['active_buttons'][] = array_merge( array( 'slug' => $button_slug ), $this->prop['default_buttons'][ $button_slug ] );
			endif;
		endforeach;
		foreach ( $this->prop['default_buttons'] as $button_slug => $button_prop ) :
			if ( ! in_array( $button_slug, $toolbar_conf->my_buttons, true ) ) :
				$this->prop['unused_buttons'][] = array_merge( array( 'slug' => $button_slug ), $button_prop );
			endif;
		endforeach;
		// Add a few pipes.
	...
	/**
	 * Generate the button labels for the correct languages after the WordPress framework has been loaded
	 *
	 * @return boolean TRUE in case of success, FALSE if the labels are already loaded
	 */
	private function make_translations() {
		...
		$ext_translations                             = apply_filters( 'markup_markdown_button_translations', array() );
