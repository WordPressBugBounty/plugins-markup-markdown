<?php
/**
 * Stable addon "Code Highlighter" to highlight code snippets in the code
 *
 * @category   Addons
...
/**
 * Class to load Prismjs or HighlightJS to enable the syntax highlighting with code snippets
 */
final class CodeHighlighter {
	...
	public function __construct() {
		if ( ! defined( 'MARKUP_MARKDOWN_ADDONS' ) || ( defined( 'MARKUP_MARKDOWN_ADDONS' ) && in_array( $this->prop['slug'], MARKUP_MARKDOWN_ADDONS, true ) === false ) ) :
			$this->prop['active'] = 0;
		else :
			if ( is_admin() ) :
				$this->init_backend();
			endif;
			if ( $this->prop['active'] > 0 && defined( 'MARKUP_MARKDOWN_USE_CODEHIGHLIGHT' ) && isset( MARKUP_MARKDOWN_USE_CODEHIGHLIGHT[0] ) && 1 === (int) MARKUP_MARKDOWN_USE_CODEHIGHLIGHT[0] ) :
				$this->init_frontend();
			endif;
		endif;
	}
	...
	private function init_frontend() {
		...
		$this->prop['engine'] = MARKUP_MARKDOWN_USE_CODEHIGHLIGHT[1];
		$this->prop['theme']  = isset( MARKUP_MARKDOWN_USE_CODEHIGHLIGHT[3] ) ? MARKUP_MARKDOWN_USE_CODEHIGHLIGHT[3] : 'vs';
		$this->plugin_uri     = markup_markdown()->plugin_uri;
		if ( ! isset( MARKUP_MARKDOWN_USE_CODEHIGHLIGHT[2] ) || (int) MARKUP_MARKDOWN_USE_CODEHIGHLIGHT[2] < 1 ) :
			return false;
		endif;
	...
	public function update_config( $my_cnf ) {
		...
		$my_cnf['code_highlighter_active'] = isset( $my_cnf['code_highlighter_engine'] ) && false !== in_array( $my_cnf['code_highlighter_engine'], array( 'prism', 'highlight' ), true ) ? 1 : 0;
	...
	public function create_const( $my_cnf ) {
		$my_cnf['MARKUP_MARKDOWN_USE_CODEHIGHLIGHT'] = array(
			isset( $my_cnf['code_highlighter_active'] ) && (int) $my_cnf['code_highlighter_active'] > 0 ? 1 : 0,
		);
		unset( $my_cnf['code_highlighter_active'] );
		if ( $my_cnf['MARKUP_MARKDOWN_USE_CODEHIGHLIGHT'][0] > 0 ) :
			$my_cnf['MARKUP_MARKDOWN_USE_CODEHIGHLIGHT'][1] = isset( $my_cnf['code_highlighter_engine'] ) ? htmlspecialchars( $my_cnf['code_highlighter_engine'] ) : '';
			$my_cnf['MARKUP_MARKDOWN_USE_CODEHIGHLIGHT'][2] = 1;
			$my_cnf['MARKUP_MARKDOWN_USE_CODEHIGHLIGHT'][3] = isset( $my_cnf['code_highlighter_theme'] ) ? htmlspecialchars( $my_cnf['code_highlighter_theme'] ) : '';
		endif;
	...
	public function add_tabcontent() {
		...
		$my_tmpl = markup_markdown()->plugin_dir . '/MarkupMarkdown/Addons/Released/Templates/tmpl-codehighlighterform.php';
		if ( markup_markdown()->exists( $my_tmpl ) ) :
			markup_markdown()->clear_cache( $my_tmpl );
			$this->get_themes();
			$markup_markdown_code_themes = $this->themes;
			$this->themes                = array();
			$markup_markdown_user_theme  = $this->prop['theme'];
			include $my_tmpl;
		endif;
	}
	 ...
	 * @access public
	 *
	 * @return boolean TRUE if an asset has been queued, FALSE otherwise
	 */
	public function load_front_highlighter_stylesheets() {
		...
		elseif ( 'prism' === $this->prop['engine'] ) :
			if ( file_exists( markup_markdown()->plugin_dir . 'assets/prism/v1/themes/' . $this->prop['theme'] . '.min.css' ) ) :
				wp_enqueue_style( 'markup_markdown__prism_theme', markup_markdown()->plugin_uri . 'assets/prism/v1/themes/' . $this->prop['theme'] . '.min.css', array(), '1.30.1001' );
				return true;
			endif;
		elseif ( 'highlight' === $this->prop['engine'] ) :
			if ( file_exists( markup_markdown()->plugin_dir . 'assets/highlightjs/styles/' . $this->prop['theme'] . '.min.css' ) ) :
				wp_enqueue_style( 'markup_markdown__hl_theme', markup_markdown()->plugin_uri . 'assets/highlightjs/styles/' . $this->prop['theme'] . '.min.css', array(), '11.11.1001' );
				return true;
			endif;
		endif;
		return false;
	 ...
	 * @access public
	 *
	 * @return boolean TRUE if js assets have been queued, FALSE otherwise
	 */
	public function load_front_highlighter_scripts() {
		...
		if ( ! isset( $this->prop['engine'] ) || empty( $this->prop['engine'] ) || 'none' === $this->prop['engine'] ) :
			return false; // Do nothing.
		elseif ( 'prism' === $this->prop['engine'] ) :
			wp_enqueue_script( 'markup_markdown__prism_core', $plugin_uri . 'assets/prism/v1/components/prism-core.js', array(), '1.30.1001', true );
			...
			return true;
		elseif ( 'highlight' === $this->prop['engine'] ) :
			wp_enqueue_script( 'markup_markdown__hl_core', $plugin_uri . 'assets/highlightjs/highlight.min.js', array(), '11.11.1', true );
	 ...
	 * @access public
	 *
	 * @return string Prism inline JS setting to be added
	 */
	public function add_inline_prism_conf() {
		return 'Prism.plugins.autoloader.languages_path = "' . markup_markdown()->plugin_uri . '/assets/prism/v1/components/";';
	}
	 ...
	 * @access public
	 *
	 * @return string Highlight inline JS setting to be added
	 */
	...
	/**
	 * Retrieve Prism / HighlightJS themes options
	 *
	 * @return TRUE if the skins info has been loaded, FALSE if already loaded
	 */
	public function get_themes() {
		...
		$this->themes = array(
			'prismjs'     => array(
				...
				'prism-nord'                            => __( 'Nord', 'markup-markdown' ),
			...
			'highlightjs' => array(
				...
				'hl-foundation'                           => __( 'Foundation', 'markup-markdown' ),
