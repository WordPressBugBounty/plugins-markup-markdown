<?php
/**
 * Base methods to handle a few oembed services
 *
 * @category   Abstracts
...
/**
 * Mostly helpers to extract data from oembed requests
 */
abstract class OEmbedTinyAPI {
	 ...
	 * @since 2.0.0
	 *
	 * @return array{content: string, endpoint: string, rexexp: string} $ops The options to be used.
	 ...
	 */
	protected function oembed_service( $ops = array() ) {
	...
	/**
	 * Verify parts of the media's url for specific param
	 *
	 * @param string $target_media The target media string.
	 * @return array{original: string, url: string, options: string} Setup of the media
	 */
	 ...
	 * @return string The oEmbed modified reponse object
	 */
	...
	/**
	 * Retrieve the data from a remote endpoint to generate and get the appropriate iframe HTML snippet
	 *
	 * @param string $api_endpoint URL of the target endpoint.
	 * @param string $provider_name Slug of the provider.
	 * @return object The body oject
	 */
	...
	protected function format_standalone_iframes( $content = '', $provider_name = '' ) {
		...
		foreach ( $standalone_iframes[1] as $iframe_idx => $iframe_html ) :
			$tmp_args  = array();
			$my_iframe = array( 'html' => $iframe_html );
	 ...
	 *
	 * @return array $ops Media options.
	...
	protected function format_medias( $ops ) {
		...
		endforeach;
		return defined( 'MARKUP_MARKDOWN_USE_BLOCKSTYLES' ) && MARKUP_MARKDOWN_USE_BLOCKSTYLES ? $this->format_standalone_iframes( $my_content ) : $my_content;
	}
